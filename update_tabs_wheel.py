...
export function ResearchTabNav({ runId }: { runId: string }) {
  ...
  // Enable Mouse Wheel (Vertical Scroll) to translate into Horizontal Scroll
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;

    const handleWheel = (e: WheelEvent) => {
      // If there's vertical scroll delta, convert it to horizontal scroll on our container
      if (e.deltaY !== 0) {
    ...
    // passive: false is necessary to be able to call preventDefault
...
print("Added MouseWheel listener to ResearchTabNav.tsx")
