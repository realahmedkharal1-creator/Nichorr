...
export default function KnowledgeGraphExplorerPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto py-4 font-sans">
      {/* Back Button */}
      <Link href={`/projects/${params.id}`} className="text-xs font-mono text-slate-400 hover:text-slate-200 flex items-center gap-1">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div>
          <span className="text-xs font-mono text-indigo-400 font-semibold uppercase tracking-wider block mb-1">EVIDENCE GRAPH VISUALIZER</span>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">Trace relational links between Research Runs, Sources, Evidence Excerpts, Verified Claims, and Content Items.</p>
        </div>
        ...
        >
          <RefreshCw className="w-4 h-4 text-indigo-400" /> Reload Graph
        </button>
      </div>

      {/* Filter Toolbar */}
      <div className="slate-card p-4 bg-slate-900/80 border-slate-800 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs font-mono text-slate-400">
          <Filter className="w-4 h-4 text-indigo-400" /> Filter Node Type:
        </div>
      ...
      <div className="slate-card p-8 bg-slate-950/90 border-slate-800 min-h-[320px] flex flex-col items-center justify-center relative overflow-hidden space-y-4">
        ...
        <div className="text-center relative z-10 space-y-1">
          <h3 className="text-base font-bold text-slate-100">Interactive Relational Graph Mesh</h3>
          <p className="text-xs text-slate-400 max-w-md">Displaying {filteredNodes.length} active Graph Nodes and {edges.length} Relational Claims Edges in this project workspace.</p>
        </div>

        {/* Node Badges */}
        <div className="flex flex-wrap justify-center gap-2 relative z-10 max-w-xl">
          {filteredNodes.map((n) => (
            <span key={n.id} className="px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-750 text-xs font-mono text-slate-200 shadow-sm flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-indigo-400" />
              <strong className="text-indigo-300">[{n.node_type}]</strong> {n.label}
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h2 className="text-base font-bold text-slate-100">Knowledge Graph Nodes List</h2>

        {loading ? (
          <SkeletonCard />
        ) : filteredNodes.length === 0 ? (
          <div className="slate-card p-8 text-center text-slate-500 italic text-xs">
              ...
              <tbody className="divide-y divide-slate-850">
                {filteredNodes.map((n) => (
                  <tr key={n.id} className="hover:bg-slate-900/50">
                    <td className="p-3 font-semibold text-slate-100">{n.label}</td>
                    <td className="p-3 font-mono text-indigo-400">{n.node_type}</td>
