...
export async function GET(
  ...
  try {
    ...
    const changes = PublicationIntegrityProvider.getChanges(params.publicationId, run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load publication changes" }, { status: 500 });
  }
}
