...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const health = PublicationIntegrityProvider.getHealthReport(run.id, userId);
    const publications = PublicationIntegrityProvider.getPublications(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load publication integrity" }, { status: 500 });
  }
}
