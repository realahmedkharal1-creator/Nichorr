...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = PublicationIntegrityProvider.reconcilePublications(run.id, userId, body);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to reconcile publications" }, { status: 500 });
  }
}
