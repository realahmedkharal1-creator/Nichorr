...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const history = PublicationIntegrityProvider.getHistory(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load publication integrity history" }, { status: 500 });
  }
}
