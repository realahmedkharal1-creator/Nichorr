...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const cancelled = CreatorExportProvider.cancelExport(body.packageId || pkg.packageId, run.id, userId);

    return NextResponse.json({
      success: cancelled,
    });
  } catch (err: any) {
    return NextResponse.json({ success: false, error: err.message || "Failed to cancel export" }, { status: 500 });
  }
}
