...
export async function GET(
  ...
  try {
    ...
    const health = ResearchHealthProvider.evaluateHealth(run, report);
    const claimRecord = health.claimsHealth.find((c) => c.claimId === params.claimId);

    if (!claimRecord) {
      return NextResponse.json({ success: false, error: "Claim not found in research health report" }, { status: 404 });
    }

    const upstreamEvidenceDetails = claimRecord.upstreamEvidenceIds
      .map((id) => health.evidenceHealth.find((e) => e.evidenceId === id))
      .filter(Boolean);

    return NextResponse.json({
      success: true,
      claim: claimRecord,
      upstreamEvidence: upstreamEvidenceDetails,
      topic: run.topic,
      lastCheckedAt: health.lastCheckedAt,
      evidenceSnapshotHash: health.evidenceSnapshotHash,
    });
  } catch (error: any) {
