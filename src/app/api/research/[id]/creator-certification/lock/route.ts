...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const lockResult = CreatorCertificationProvider.applyReleaseLock(certificate, userId, body.notes);

    if (!lockResult.success) {
      return NextResponse.json({ success: false, error: lockResult.errorMessage }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to apply Release Lock" }, { status: 500 });
  }
}
