...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const certificate = CreatorCertificationProvider.getCertificate(run.id, userId);

    const changes = CreatorCertificationProvider.detectChangesSinceCertification(
      snapshot,
      certificate
    );
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to detect changes since certification" }, { status: 500 });
  }
}
