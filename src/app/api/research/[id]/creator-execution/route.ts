...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const activePlan = CreatorExecutionProvider.getActivePlan(run.id, userId);
    const stagedExecution = CreatorExecutionProvider.getStagedExecution(run.id, userId);
    const history = CreatorExecutionProvider.getHistory(run.id, userId);

    return NextResponse.json({
      success: true,
      activePlan: activePlan || null,
      stagedExecution: stagedExecution || null,
      executionHistoryCount: history.length,
      currentScriptVersion: report.scriptVersion || 1,
    });
  } catch (error: any) {
