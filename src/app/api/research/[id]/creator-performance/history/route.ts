...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const history = PerformanceProvider.getHistory(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load performance history" }, { status: 500 });
  }
}
