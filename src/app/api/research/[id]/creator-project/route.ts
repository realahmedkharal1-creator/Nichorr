...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const projectOverview = CreatorProjectProvider.getProjectOverview(
      run,
      report,
      DEFAULT_PRODUCTION_PREFERENCES,
      profile,
    ...
    return NextResponse.json({
      ...
      snapshot: projectOverview.snapshot,
      health: projectOverview.healthReport,
      pipelineStages: projectOverview.pipelineStages,
      assets: projectOverview.assets,
      graph: projectOverview.graph,
    });
  } catch (error: any) {
