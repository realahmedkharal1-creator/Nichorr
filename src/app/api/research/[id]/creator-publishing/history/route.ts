...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const history = CreatorPublishingProvider.getHistory(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load publishing history" }, { status: 500 });
  }
}
