...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const receipts = CreatorPublishingProvider.getReceipts(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load receipts" }, { status: 500 });
  }
}
