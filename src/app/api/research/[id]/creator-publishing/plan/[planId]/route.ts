...
export async function GET(
  ...
  try {
    ...
    if (plan.planId !== params.planId) {
      return NextResponse.json({ success: false, error: "Publishing plan not found." }, { status: 404 });
    }
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load publishing plan" }, { status: 500 });
  }
}
