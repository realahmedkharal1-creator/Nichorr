...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { platform, note, packageVersion } = body;

    if (!platform) {
      return NextResponse.json({ success: false, error: "Missing required platform parameter" }, { status: 400 });
    }
    ...
    const approveResult = DistributionProvider.approveTarget(distPackage, platform, note, userId);

    if (!approveResult.success) {
      return NextResponse.json({ success: false, error: approveResult.errorMessage }, { status: 400 });
    }

    return NextResponse.json({
      success: true,
      distributionPackage: approveResult.updatedPackage,
    });
  } catch (error: any) {
