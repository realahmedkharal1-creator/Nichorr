...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { platform, reason, packageVersion } = body;

    if (!platform || !reason) {
      return NextResponse.json({ success: false, error: "Missing required platform or reason parameter" }, { status: 400 });
    }
    ...
    const rejectResult = DistributionProvider.rejectTarget(distPackage, platform, reason, userId);

    return NextResponse.json({
      success: true,
      distributionPackage: rejectResult.updatedPackage,
    });
  } catch (error: any) {
