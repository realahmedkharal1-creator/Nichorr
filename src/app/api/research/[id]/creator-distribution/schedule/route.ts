...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { platform, localDateTime, timezone, note, packageVersion } = body;

    if (!platform || !localDateTime || !timezone) {
      return NextResponse.json({
        success: false,
        error: "Missing required parameters: platform, localDateTime, and timezone are mandatory for scheduling.",
      }, { status: 400 });
    }

    let report = run.creatorStudio;
    if (!report) {
      report = CreatorStudioProvider.generateReport(run, 12, DEFAULT_PRODUCTION_PREFERENCES, undefined, "SCRIPT_READY");
      run.creatorStudio = report;
      ResearchEngine.setRun(run);
    }

    const preflight = PublishingProvider.runPreflight(run, report, DEFAULT_PRODUCTION_PREFERENCES);
    ...
    const scheduleResult = DistributionProvider.scheduleTarget(
      distPackage,
      platform,
      localDateTime,
      timezone,
    ...
    if (!scheduleResult.success) {
      return NextResponse.json({ success: false, error: scheduleResult.errorMessage }, { status: 400 });
    }

    return NextResponse.json({
      success: true,
      distributionPackage: scheduleResult.updatedPackage,
    });
  } catch (error: any) {
