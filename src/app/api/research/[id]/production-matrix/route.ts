...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const matrix = ProductionMatrixProvider.getProductionMatrix(run.id, userId, EVIDENCE_SNAPSHOT_HASH);

    return NextResponse.json({ success: true, matrix });
  } catch (err: any) {
    return NextResponse.json(
      { success: false, error: err.message || "Failed to load production matrix" },
      { status: 500 }
    );
  }
}
