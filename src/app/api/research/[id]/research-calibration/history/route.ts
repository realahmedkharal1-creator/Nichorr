...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const history = ResearchCalibrationProvider.getHistory(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load calibration history" }, { status: 500 });
  }
}
