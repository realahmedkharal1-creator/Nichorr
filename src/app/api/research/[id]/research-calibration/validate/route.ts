...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { queueItemId, ...options } = body;

    if (!queueItemId) {
      return NextResponse.json({ success: false, error: "queueItemId is required" }, { status: 400 });
    }

    const result = ResearchCalibrationProvider.validateQueueItem(queueItemId, run.id, userId, options);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to validate calibration item" }, { status: 500 });
  }
}
