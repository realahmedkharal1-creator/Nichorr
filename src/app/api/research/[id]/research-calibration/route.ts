...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const queue = ResearchCalibrationProvider.getQueue(run.id, userId);
    const snapshot = ResearchCalibrationProvider.getSnapshot(run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load research calibration" }, { status: 500 });
  }
}
