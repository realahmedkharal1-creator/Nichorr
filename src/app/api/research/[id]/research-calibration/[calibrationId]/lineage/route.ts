...
export async function GET(
  ...
  try {
    ...
    const lineage = ResearchCalibrationProvider.getLineage(params.calibrationId, run.id, userId);
    ...
    return NextResponse.json({ success: false, error: err.message || "Failed to load calibration lineage" }, { status: 500 });
  }
}
